...
public class VardiyaGunHome extends EntityHome<VardiyaPlan> implements Serializable {
	...
	private Boolean topluFazlaCalismaTalep = Boolean.FALSE, denklestirmeAyDurum = Boolean.FALSE, fazlaMesaiTalepDurum = Boolean.FALSE;
	...
	public String mesaiMudurOnayCevabi(boolean onayDurum) {
		...
		if (mailSatu != null && mailSatu.isDurum() && authenticatedUser == null)
		...
		for (Iterator<String> iterator = vardiyaGunMap.keySet().iterator(); iterator.hasNext();) {
			...
			if (vardiyaGun.getVardiya() != null) {
				...
				if (iseBaslamaTarihi == null)
					iseBaslamaTarihi = vardiyaGun.getPersonel().getIseBaslamaTarihi();
		...
		if (yaz) {
			...
			for (int i = 1; i <= vardiyaHaftaList.size(); i++) {
				...
				if (iseBaslamaTarihi == null && vardiyaHafta.getPersonel() != null)
					iseBaslamaTarihi = vardiyaHafta.getPersonel().getIseBaslamaTarihi();
	...
	private void aylikVardiyaOzetOlustur() {
		...
		for (AylikPuantaj aylikPuantaj : aylikPuantajList) {
			...
			aylikPuantaj.setOnayDurum(!(aylikPuantaj.getPersonelDenklestirmeAylik() != null && aylikPuantaj.getPersonelDenklestirmeAylik().isOnaylandi()));
	...
	private void vardiyalarSec(AylikPuantaj aylikPuantaj) {
		if (aylikPuantaj != null) {
			...
			for (VardiyaGun pdksVardiyaGun : aylikPuantaj.getVardiyalar()) {
				if (pdksVardiyaGun.getVardiya() != null) {
					ArrayList<Vardiya> list = pdksVardiyaGun.getIzin() == null || (pdksVardiyaGun.getVardiya().isFMI() && fazlaMesaiIzinRaporuDurum) ? vardiyalar : null;
	...
	private void aylikPuantajOlusturuluyor() {
				...
				try {
					...
					if (aramaSecenekleri.getEkSaha3Id() != null && !personelList.isEmpty() && !gorevliPersonelMap.isEmpty()) {
						...
						for (Iterator iterator = personelList.iterator(); iterator.hasNext();) {
							...
							if (personel.getEkSaha3() == null || personel.getIseBaslamaTarihi() == null || personel.getIstenAyrilisTarihi() == null)
								iterator.remove();
		...
		if (personelList != null && !personelList.isEmpty()) {
			...
			for (Personel personel : personelList) {
				...
				long iseBasTarih = Long.parseLong(PdksUtil.convertToDateString(personel.getIseBaslamaTarihi(), "yyyyMMdd"));
				...
				if (gecenAylikPuantajSablon.getIlkGun().getTime() <= personel.getSonCalismaTarihi().getTime() && gecenAylikPuantajSablon.getSonGun().getTime() >= personel.getIseBaslamaTarihi().getTime()) {
		...
		topluFazlaCalismaTalep = fazlaMesaiTalepVar && denklestirmeAyDurum && aylikPuantajList.size() > 0 && (ortakIslemler.getTestDurum() || authenticatedUser.isAdmin());
		int adet = 0;
		for (Iterator iterator = aylikPuantajList.iterator(); iterator.hasNext();) {
			AylikPuantaj aylikPuantaj = (AylikPuantaj) iterator.next();
	...
	private void vardiyaHareketKontrol(AylikPuantaj aylikPuantajSablon) {
		if (denklestirmeAy != null && denklestirmeAy.getDurum() && aylikPuantajList != null && aylikPuantajList.size() > 0) {
	...
	public String vardiyaPlanDosyaYaz() throws Exception {
		...
		for (AylikPuantaj aylikPuantaj : aylikPuantajDosyaList) {
			if (aylikPuantaj.isKaydet() && aylikPuantaj.isSecili()) {
				...
				boolean calisiyor = personel.getIseBaslamaTarihi().before(aylikPuantajDefault.getIlkGun()) && personel.getIstenAyrilisTarihi().after(sonGun);
						...
						if (personelMap.size() == perMap.size()) {
							...
							for (String sicilNo : perList) {
								...
								if (personel != null) {
									...
									personel.setDurum(Boolean.TRUE);
									boolean calisiyor = personel.getIseBaslamaTarihi().before(aylikPuantajSablon.getIlkGun()) && personel.getIstenAyrilisTarihi().after(sonGun);
	...
	private boolean haftalikSablonOlustur(AylikPuantaj aylikPuantajSablon, boolean fiush, boolean pdksVardiyaHaftaSave, TreeMap<String, VardiyaHafta> vardiyaHaftaMap, Personel personelGelen, VardiyaSablonu sablonu, AylikPuantaj aylikPuantaj, TreeMap<String, VardiyaGun> vardiyaGunAllMap) {
		...
		personel.setDurum(Boolean.TRUE);
		boolean calisiyorPersonel = aylikPuantajSablon.getSonGun().after(personel.getIseBaslamaTarihi()) && personel.getIstenAyrilisTarihi().after(aylikPuantajSablon.getIlkGun());
