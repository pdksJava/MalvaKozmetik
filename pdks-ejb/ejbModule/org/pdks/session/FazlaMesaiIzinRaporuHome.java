...
public class FazlaMesaiIzinRaporuHome extends EntityHome<VardiyaGun> implements Serializable {
	...
	public String vardiyaOzetExcel() {
		try {
			...
			if (baosDosya != null) {
				...
				String dosyaAdi = PdksUtil.setTurkishStr("FazlaMesaiIzinRaporu" + PdksUtil.convertToDateString(date, "yyyyMMdd")) + ".xlsx";
		...
		try {
			...
			if (izinVardiyaGunList.size() > 0) {
				...
				for (VardiyaGun vg : izinVardiyaGunList) {
					...
					col = vardiyaSatirEkle(sheet, col, row, satirStyle, satirCenter, vg);
					renk = !renk;
				}
	 ...
	 * @param vg
	 */
	private int vardiyaSatirEkle(Sheet sheet, int col, int row, XSSFCellStyle satirStyle, XSSFCellStyle satirCenter, VardiyaGun vg) {
